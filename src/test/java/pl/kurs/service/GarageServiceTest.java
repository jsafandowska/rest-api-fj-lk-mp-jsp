...
public class GarageServiceTest {
    ...
    void shouldReturnAllGarages() {
        ...
        verify(garageRepository, times(0)).findAll();
    }
    ...
    public void shouldAddCarToGarage() {
        when(garageRepository.findById(1)).thenReturn(Optional.of(garage2));
        when(carRepository.findById(1)).thenReturn(Optional.of(car));

        garageService.addCarToGarage(1, 1);

//        assertEquals(garage1.getCars().size(), 1);
        assertTrue(garage1.getCars().contains(car)); // Sprawdzenie, czy konkretny samochód jest w garażu
        assertEquals(car.getGarage(), garage2); // Sprawdzenie, czy samochód ma ustawiony poprawny garaż
        verify(garageRepository, times(1)).findById(1);
        verify(carRepository, times(1)).findById(1);
        verify(garageRepository, times(1)).saveAndFlush(garage1);
