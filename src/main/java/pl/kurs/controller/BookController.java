...
public class BookController {
    ...
    @PostConstruct
    public void init() {
        AuthorDto a1 = authorService.addAuthor(new CreateAuthorCommand("Kazimierz", "Wielki", 1900, 2000));
        AuthorDto a2 = authorService.addAuthor(new CreateAuthorCommand("Maria", "Wielka", 1900, 2000));

        bookService.addBook(new CreateBookCommand("Ogniem i mieczem", "LEKTURA", a1.id()));
        bookService.addBook(new CreateBookCommand("Ogniem i mieczem 2", "LEKTURA", a2.id()));
    }
    ...
    public ResponseEntity<BookDto> editBookPartially(@PathVariable int id, @RequestBody EditBookCommand command) {
        log.info("editBookPartially({}, {})", id, command);
